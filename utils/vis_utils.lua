...
--  print("boxes", boxes)
  ...
  for i = 1, N do
    ...
    local ok, err = pcall(function()
      image.drawText(text_img, tostring(captions[i]), x, y, text_opt)
    end)
    if not ok then
...
function vis_utils.visualize(img,boxes_table,score_table,w,ow,dir,idx)
   for k,v in pairs(boxes_table) do
     if k ~= 1 then
       if v:dim() ~= 0 then 
          local b = box_utils.xcycwh_to_xywh(v)
          b =  box_utils.scale_boxes_xywh(b:float(), ow/w)
          print(v:size())
          img = vis_utils.densecap_draw(img,b,score_table[k])
       end
     end
   end
   local iname = dir .. tostring(idx) .. ".jpg"
   image.save(iname,img)
end
