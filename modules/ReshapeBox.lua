local layer, parent = torch.class('nn.ReshapeBox', 'nn.Module')
...
function layer:updateOutput(input)
  ...
  --self.input_perm:copy(input:view(N, k, D, H, W):permute(1,3,4,5,2))
  --self.output = self.input_perm:view(N, k * H * W, D)
  self.output = input:permute(1,3,4,2):contiguous()
  self.output = self.output:view(N,k*H*W,D)
  return self.output
end
